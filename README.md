...
- 基于 `termios` 配置串口，`O_NONBLOCK` 非阻塞打开
- `epoll` 监听可读事件，与 `eventfd` 组合实现优雅退出
- 粘包拆包：按帧头（`0xAA 0x55`）+ 长度字段滑动切割
- CRC16-CCITT 校验（初值 `0xFFFF`，多项式 `0x1021`）
- 独立的心跳线程，固定周期发送 `HEARTBEAT` 帧
- `send_frame()` 加互斥锁，线程安全
...
| 第二阶段 | 串口通信：termios 配置、epoll 收发、CRC16、粘包拆包、心跳 | 🔧 进行中 |
